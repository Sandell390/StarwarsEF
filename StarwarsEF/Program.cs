...
List<Planet> planet1 = planets.IntersectBy(new List<string>(){"Mon Cala", "Malastare","Mirial","Muunilinst","Mustafar","Mygeeto"}, planet => planet.Name).ToList();

List<Planet> planet2 = planets.Where(planet => planet.Name.Contains("Y") || planet.Name.Contains("y")).ToList();

List<Planet> planet3 = planets.Where(planet => planet.Name.Length > 9 && planet.Name.Length < 15).ToList();

List<Planet> planet4 = planets.Where( planet => (planet.Name[1] == 'a' || planet.Name[1] == 'A') && (planet.Name[planet.Name.Length - 1] == 'e' || planet.Name[planet.Name.Length - 1] == 'E')).ToList();

List<Planet> planet5 = planets.Where(planet => planet.RotationPeriod > 40).OrderBy(planet => planet.RotationPeriod).ToList();

List<Planet> planet6 = planets.Where(planet => planet.RotationPeriod > 10 && planet.RotationPeriod < 20).OrderBy(planet => planet.Name).ToList();

List<Planet> planet7 = planets.Where(planet => planet.RotationPeriod > 30).OrderBy(planet => planet.Name)
    .ThenBy(planet => planet.RotationPeriod).ToList();

List<Planet> planet8 = planets
    .Where(planet => (planet.RotationPeriod < 30 || planet.SurfaceWater > 50) && planet.Name.Contains("ba"))
    .OrderBy(planet => planet.Name).ThenBy(planet => planet.SurfaceWater).ThenBy(planet => planet.RotationPeriod)
    .ToList();

List<Planet> planet9 =
    planets.Where(planet => planet.SurfaceWater > 0).OrderByDescending(planet => planet.SurfaceWater).ToList();

List<Planet> planet10 = planets.Where(planet => planet.Diameter != 0 && planet.Population != 0).OrderBy(planet => (4 * Math.PI * (planet.Diameter / 2.0 * planet.Diameter / 2.0)) / planet.Population).ToList();

List<Planet> ExceptList = planets.Where(planet => planet.RotationPeriod > 0).ToList();
List<Planet> planet11 = planets.Except(ExceptList, new PlanetComparer()).ToList();

List<Planet> planet12A = planets
    .Where(planet => planet.Name.ToLower().StartsWith("a") || planet.Name.ToLower().EndsWith("s")).ToList();

List<Planet> planet12B = planets
    .Where(planet => planet.Terrain != null && planet.Terrain.Contains("rainforests")).ToList();

List<Planet> planet12 = planet12A.Union(planet12B).ToList();

List<Planet> planet13 = planets.Where(planet => planet.Terrain != null && planet.Terrain.Exists(terrain => terrain.Contains("desert"))).ToList();

List<Planet> planet14 = planets
    .Where(planet => planet.Terrain != null && planet.Terrain.Exists(terrain => terrain.Contains("swamp")))
    .OrderBy(planet => planet.RotationPeriod).ThenBy(planet => planet.Name).ToList();

List<Planet> planet15 = planets
    .Where(planet => new Regex("[e]{2,}|[u]{2,}|[i]{2,}|[o]{2,}|[a]{2,}").IsMatch(planet.Name)).ToList();
    
    
List<Planet> planet16 = planets
    .Where(planet => new Regex("[k]{2,}|[l]{2,}|[r]{2,}|[n]{2,}").IsMatch(planet.Name)).OrderByDescending(planet => planet.Name).ToList();
...
void Print(int step,List<Planet> planetsPrint)
{
    Console.WriteLine("==================================");
    Console.WriteLine($"Step {step}");
    foreach (var planet in planetsPrint)
    {
